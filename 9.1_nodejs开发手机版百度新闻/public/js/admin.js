		...
		success:function (data) {
			...
			if(data.newsdata == "null"){
				...
				$.each(data,function(index,value) {
                    ...
                    var newstime = new Date(value.newstime); 
                        newstime = new Date(+new Date(newstime)+8*3600*1000).toISOString().replace(/T/g,' ').replace(/\.[\d]{3}Z/,'');
					var tdtime = $("<td></td>").html(newstime.split(' ')[0]);
					var tdcz = $("<td></td>");
...
function addNews() {
    $("#submit").click(function(e) {
        ...
        // 获得表单数据		
        var jsonNews = {
                "newstitle": noHtml(titleId.val()),
                "newstype": typeId.val(),
                "newsimg": imgId.val(),
                "newstime": timeId.val(),
                "newssrc": noHtml(srcId.val()),
                "onfocus": onfocusval,
            }
    ...
    $("#newstable tbody").on("click", ".btn-primary", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var newstime = new Date(data[0].newstime); 
                    newstime = new Date(+new Date(newstime)+8*3600*1000).toISOString().replace(/T/g,' ').replace(/\.[\d]{3}Z/,'');
		        $("#upnewstime").val(newstime.split(' ')[0]);
    ...
    $("#updateModal .btn-primary").click(function() {
    	...
    	$.ajax({
            ...
            success: function(data) {
                ...
                //发送数据后，刷新新闻列表
                refreshNews(upjsonNews.newstype);
            }
        });
...
/**
 * 内容入库时过滤
 * @param  {[type]} value 传入值
 * @return {[type]} 
 */
function noHtml(value){
    value=value.replace(/({|})/g,'');   //过滤{}
    value=value.replace(/[ | ]*\n/g,'\n'); //去除行尾空白
    value=value.replace(/\n[\s| | ]*\r/g,'\n'); //去除多余空行
    value=value.replace(/<\/?[^>]*>/g,''); //*<\/?[^>]*>可以匹配<script></style></body>等，并置空。而不是替换<和>两个符号
    value=value.replace(/</g,'&lt;');    //置换符号<
    return value.replace(/>/g,'&gt;');    //置换符号>
